...
describe('Login signup captcha (Type 67) integration', () => {
  ...
  it('opens captcha from signup and verifies to set Verified state', () => {
    ...
    const verifyBtn = screen.getByRole('button', { name: /verify human/i });
  ...
  it('cancel closes captcha and keeps unverified state', () => {
    render(<Login isOpen={true} onClose={onClose} />);
    ...
    // Open captcha
    fireEvent.click(screen.getByRole('button', { name: /verify human/i }));
