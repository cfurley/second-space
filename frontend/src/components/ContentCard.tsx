...
export function ContentCard({ type, content }: ContentCardProps) {
  ...
  return (
    <div
      className="bg-white/5 border border-white/10 rounded-xl p-5 text-white transition-all duration-200 cursor-pointer hover:bg-white/8 hover:border-white/20 hover:-translate-y-0.5"
      data-type={type}                               // NEW (QA-friendly)
      aria-label={`media-card-${type}`}              // NEW (a11y/test)
    >
      {getCardContent()}
